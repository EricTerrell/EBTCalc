...
module.exports = class BuiltInFunctions {
    ...
    _fix(decimalDigits) {
        ...
        if (decimalDigits < 0 || decimalDigits > 100) {
            throw 'Error: Fix argument must be between 0 and 100.';
        }
    ...
    _processVariableUI(executeFunctionItem, windowOptions) {
        const window = WindowUtils.createWindow('variable', true,() => { this._enableDisableFunctionButtons(); });
        ...
        window.webContents.once(StringLiterals.DID_FINISH_LOAD, () => {
            window.webContents.send(StringLiterals.VARIABLE_WINDOW_CHANNEL, windowOptions);
        });
    ...
    _promptForString() {
        ...
        const window = WindowUtils.createWindow('prompt', true,() => {button.disabled = false});
        ...
        window.webContents.once(StringLiterals.DID_FINISH_LOAD, () => {
            window.webContents.send(StringLiterals.VARIABLE_WINDOW_CHANNEL, options);
        });
    }
