...
module.exports = class Category {
    ...
    _getCategoryData(extract) {
        const buttonRegEx = /button\s+([\S]+)\s+("[^"]*")\s+("[^"]*")/i;
    ...
    _edit(offset = 0) {
        ...
        const window = WindowUtils.createWindow('edit', true,() => {this._contextMenuActive = false});
        ...
        window.webContents.once(StringLiterals.DID_FINISH_LOAD, () => {
            ipc.invoke(StringLiterals.RELAY_TO_RENDERER, window.id, StringLiterals.CHILD_WINDOW_CHANNEL, offset).then();
        });
    }
